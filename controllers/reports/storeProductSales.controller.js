...
function getList(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const store = req.getValue('store')
      if (!store) return reject(`store required`)
      ...
      if (ayarlar.connector.connectionType == 'mssql') {
        if (ayarlar.connector.mssql.mainApp == 'mikro_v16' || ayarlar.connector.mssql.mainApp == 'mikro_v17') {
          mikroHelper.storeProductSales(dbModel, sessionDoc, ayarlar.connector, startDate, endDate, store)
            .then(async result => {
              resolve(result)
            })
            .catch(reject)
