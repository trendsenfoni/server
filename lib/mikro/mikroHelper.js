  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const query = `
       use ${sessionDoc.db};
     SELECT * FROM (
      SELECT dbo.KASALAR.kas_tip, dbo.KASALAR.kas_kod AS Kod, dbo.KASALAR.kas_isim AS Isim,
                              dbo.fn_KasaTipi(dbo.KASALAR.kas_tip) AS Turu,
        CASE
          WHEN Cari_F10da_detay = 1 THEN dbo.fn_CariHesapAnaDovizBakiye(kas_firma_no, 4, kas_kod, '', '', 0, NULL, NULL, 0, 0, 0, 0, 0)
          WHEN Cari_F10da_detay = 2 THEN dbo.fn_CariHesapAlternatifDovizBakiye(kas_firma_no, 4, kas_kod, '', '', 0, NULL, '2024-11-10', 0, 0, 0, 0, 0)
          WHEN Cari_F10da_detay = 3 THEN dbo.fn_CariHesapOrjinalDovizBakiye(kas_firma_no, 4,kas_kod, '', '', 0, NULL, '2024-11-10', 0, 0, 0, 0, 0)
          WHEN Cari_F10da_detay = 4 THEN dbo.fn_CariHareketSayisi(4, kas_kod, '')
        END AS Bakiye,
        dbo.fn_DovizSembolu(kas_doviz_cinsi) AS ParaBirimi
      FROM            dbo.KASALAR WITH (NOLOCK) LEFT OUTER JOIN
                              dbo.vw_Gendata ON 1 = 1
      ) X
      WHERE ABS(X.Bakiye)>5
      ORDER BY X.kas_tip
      `
      mssql(connector.clientId, connector.clientPass, connector.mssql, query)
        .then(result => {
...
exports.kasaBakiyeleriOzet = function (dbModel, sessionDoc, connector, lastDate) {
  return new Promise(async (resolve, reject) => {
    try {
      const query = `
      use ${sessionDoc.db};
      SELECT ParaBirimi + '-' + CAST(kas_tip as VARCHAR(10)) as ID,
      kas_tip,SUM(Bakiye) as Bakiye, ParaBirimi  FROM (
      SELECT dbo.KASALAR.kas_tip, dbo.KASALAR.kas_kod AS Kod, dbo.KASALAR.kas_isim AS Isim, 
                              dbo.fn_KasaTipi(dbo.KASALAR.kas_tip) AS Turu, 
        CASE 
          WHEN Cari_F10da_detay = 1 THEN dbo.fn_CariHesapAnaDovizBakiye(kas_firma_no, 4, kas_kod, '', '', 0, NULL, NULL, 0, 0, 0, 0, 0) 
...
exports.storeSalesProfit = function (dbModel, sessionDoc, connector, startDate, endDate) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      SELECT MagazaNo,Magaza,Satis,Vergi, Satis+Vergi as NetSatis, ISNULL(Maliyet,0) as Maliyet, ROUND(Satis-ISNULL(Maliyet,0),2) as Kar,
CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 1 END as KarOran, Miktar
FROM (
SELECT Y.MagazaNo, Y.Magaza, ROUND(SUM(Y.Tutar),2) as Satis,ROUND(SUM(Y.Miktar),0) as Miktar,ROUND(SUM(Y.Vergi),2) as Vergi, ROUND(SUM(CASE WHEN ISNULL(Y.SasFiyat,0)>0 THEN ISNULL(Y.SasFiyat,0) ELSE Y.SonAlis END * Y.Miktar),2) as Maliyet
FROM 
(SELECT X.MagazaNo, LOWER(X.Magaza) as Magaza, X.Kod, X.Ismi, X.Miktar, X.Tutar, X.Vergi
...
, (
	...
	FROM (
		select
			...
			sum(SH.sth_vergi *Case When sth_normal_iade=1 then -1.0 else 1.0 end) AS Vergi
...
exports.storeProductSales = function (dbModel, sessionDoc, connector, startDate, endDate, store) {
  return new Promise(async (resolve, reject) => {
    if (!startDate) return reject(`startDate required`)
    if (!endDate) return reject(`endDate required`)
    try {
      ...
      SELECT Kod,Isim,Satis,Vergi, Satis+Vergi as NetSatis, ISNULL(Maliyet,0) as Maliyet, ROUND(Satis-ISNULL(Maliyet,0),2) as Kar,
CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 1 END as KarOran, Miktar
FROM (
SELECT Y.Kod, Y.Isim, ROUND(SUM(Y.Tutar),2) as Satis,ROUND(SUM(Y.Miktar),0) as Miktar, ROUND(SUM(Y.Vergi),2) as Vergi, ROUND(SUM(CASE WHEN ISNULL(Y.SasFiyat,0)>0 THEN ISNULL(Y.SasFiyat,0) ELSE Y.SonAlis END * Y.Miktar),2) as Maliyet

FROM 
(SELECT X.Kod, LOWER(X.Isim) as Isim, X.Miktar, X.Tutar, X.Vergi
, SAS.Fiyat as SasFiyat
, (
	SELECT TOP 1 dbo.fn_StokHareketNetDeger(sth_tutar,sth_iskonto1,sth_iskonto2,sth_iskonto3,sth_iskonto4,sth_iskonto5,sth_iskonto6,
		sth_masraf1,sth_masraf2,sth_masraf3,sth_masraf4,sth_otvtutari,sth_oivtutari,sth_tip,0,sth_har_doviz_kuru,sth_alt_doviz_kuru,sth_stok_doviz_kuru) / sth_miktar
	...
	FROM (
      ...
      sum(SH.sth_vergi * Case When sth_normal_iade=1 then -1.0 else 1.0 end) AS Vergi
		...
		where (sth_tarih>='${startDate}') AND (sth_tarih<='${endDate}') AND
			(sth_evraktip in (1,3,4,13)) AND
			((sth_tip=1 AND sth_normal_iade=0) OR	(sth_tip=0 AND sth_normal_iade=1))
      AND SH.sth_cikis_depo_no=${store}
		group by SH.sth_stok_kod,ST.sto_isim, SH.sth_cikis_depo_no
	) X LEFT OUTER JOIN
	(SELECT DISTINCT
		sas_stok_kod,
...
exports.bestProductSalesProfit = function (dbModel, sessionDoc, connector, startDate, endDate, top = 10) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      SELECT TOP ${top} Kod,Isim,Satis,Vergi, Satis+Vergi as NetSatis, ISNULL(Maliyet,0) as Maliyet, ROUND(Satis-ISNULL(Maliyet,0),2) as Kar,
CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 1 END as KarOran, Miktar
FROM (
SELECT Y.Kod, Y.Isim, ROUND(SUM(Y.Tutar),2) as Satis,ROUND(SUM(Y.Miktar),0) as Miktar, ROUND(SUM(Y.Vergi),2) as Vergi, ROUND(SUM(CASE WHEN ISNULL(Y.SasFiyat,0)>0 THEN ISNULL(Y.SasFiyat,0) ELSE Y.SonAlis END * Y.Miktar),2) as Maliyet FROM 
(SELECT X.Kod, LOWER(X.Isim) as Isim, X.Miktar, X.Tutar, X.Vergi
, SAS.Fiyat as SasFiyat
, (
	SELECT TOP 1 dbo.fn_StokHareketNetDeger(sth_tutar,sth_iskonto1,sth_iskonto2,sth_iskonto3,sth_iskonto4,sth_iskonto5,sth_iskonto6,
...
exports.productMainGroupSales = function (dbModel, sessionDoc, connector, startDate, endDate) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      SELECT AnaGrup,Satis,Vergi, Satis+Vergi as NetSatis, ISNULL(Maliyet,0) as Maliyet, ROUND(Satis-ISNULL(Maliyet,0),2) as Kar,
      CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 1 END as KarOran
      FROM (
      SELECT Y.AnaGrup, ROUND(SUM(Y.Tutar),2) as Satis,ROUND(SUM(Y.Vergi),2) as Vergi, ROUND(SUM(CASE WHEN ISNULL(Y.SasFiyat,0)>0 THEN ISNULL(Y.SasFiyat,0) ELSE Y.SonAlis END * Y.Miktar),2) as Maliyet FROM 
      (SELECT X.Kod, X.AnaGrup, X.AnaGrupKod, X.Miktar, X.Tutar, X.Vergi
      , SAS.Fiyat as SasFiyat
      , (
        SELECT TOP 1 dbo.fn_StokHareketNetDeger(sth_tutar,sth_iskonto1,sth_iskonto2,sth_iskonto3,sth_iskonto4,sth_iskonto5,sth_iskonto6,
          sth_masraf1,sth_masraf2,sth_masraf3,sth_masraf4,sth_otvtutari,sth_oivtutari,sth_tip,0,sth_har_doviz_kuru,sth_alt_doviz_kuru,sth_stok_doviz_kuru) / sth_miktar
