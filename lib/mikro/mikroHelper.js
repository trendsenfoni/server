...
exports.storeSalesProfit = function (dbModel, sessionDoc, connector, startDate, endDate) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const query = `
      use ${sessionDoc.db};
      SELECT MagazaNo,Magaza,Satis, Maliyet, ROUND(Satis-Maliyet,2) as Kar,
CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 0 END as KarOran
FROM (
...
exports.bestProductSalesProfit = function (dbModel, sessionDoc, connector, startDate, endDate, top = 10) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const query = `
      use ${sessionDoc.db};
      SELECT TOP ${top} Kod,Isim,Satis, Maliyet, ROUND(Satis-Maliyet,2) as Kar,
CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 0 END as KarOran
FROM (
...
exports.productMainGroupSales = function (dbModel, sessionDoc, connector, startDate, endDate) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      SELECT AnaGrup,Satis, Maliyet, ROUND(Satis-Maliyet,2) as Kar,
      CASE WHEN ISNULL(Maliyet,0)>0 THEN ROUND((Satis-Maliyet)/Maliyet,2) ELSE 0 END as KarOran
      FROM (
